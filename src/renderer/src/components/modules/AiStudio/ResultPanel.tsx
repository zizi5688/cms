...
function PreviewCandidate({
  ...
  return (
    ...
    >
      <div
        className="relative aspect-[3/4] overflow-hidden rounded-xl border border-zinc-800 bg-zinc-950 bg-cover bg-center"
      ...
      >
        ...
        <div
          className={cn(
            ...
            active ? 'border border-emerald-400/20 bg-emerald-400/15 text-emerald-200' : 'border border-white/10 bg-black/35 text-zinc-300'
          )}
        >
...
function ResultPanel({ state }: { state: UseAiStudioStateResult }): React.JSX.Element {
  ...
  const heroAsset = useMemo(() => {
    return outputAssets.find((asset) => asset.id === heroAssetId) ?? selectedAssets[0] ?? outputAssets[0] ?? null
  }, [heroAssetId, outputAssets, selectedAssets])
  ...
  const handleSendToImageLab = (): void => {
    ...
    setMaterialImport(selectedAssets.map((asset) => asset.filePath), 'aiStudio')
    setActiveModule('material')
    addLog(`[AI Studio] 已送入 ImageLab：${selectedAssets.length} 张结果，任务 ${task.productName || task.id}`)
  }
  ...
  const handleDiscard = async (): Promise<void> => {
    ...
    addLog(`[AI Studio] 已丢弃当前选择：${selectedAssets.length} 张，任务 ${task.productName || task.id}`)
  }
    ...
    return (
      <div className="flex h-full flex-col gap-4">
        <div className="rounded-2xl border border-zinc-800 bg-zinc-950/70 p-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <div className="mt-2 text-lg font-medium text-zinc-100">{task.productName || '未命名任务'}</div>
            </div>
            <div className="rounded-full border border-zinc-800 px-3 py-1 text-xs text-zinc-400">{task.costLabel}</div>
          </div>
        </div>
        ...
        <div className="flex flex-1 flex-col items-center justify-center rounded-2xl border border-dashed border-zinc-800 bg-black/20 p-8 text-center">
          ...
          <div className="text-base text-zinc-200">{task.status === 'running' ? '结果生成中…' : '等待生成结果'}</div>
          <div className="mt-2 max-w-md text-sm text-zinc-500">
            {task.remoteTaskId
              ? `远端任务 ID：${task.remoteTaskId}`
              : '开始生成后，候选图会在这里以大图 + 缩略网格的方式呈现。'}
          </div>
        </div>
      </div>
    )
  ...
  const heroSrc = toFileSrc(heroAsset?.previewPath ?? heroAsset?.filePath)
    ...
    <div className="flex h-full min-h-0 flex-col gap-4">
      <div className="rounded-2xl border border-zinc-800 bg-zinc-950/70 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-zinc-400">
            ...
            <div className="rounded-full border border-zinc-800 px-3 py-1">候选 {outputAssets.length}</div>
            <div className="rounded-full border border-zinc-800 px-3 py-1">已选 {selectedAssets.length}</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid min-h-0 flex-1 gap-4 xl:grid-cols-[minmax(0,1.2fr)_360px]">
        <div className="flex min-h-0 flex-col gap-4 rounded-2xl border border-zinc-800 bg-zinc-950/70 p-4">
          <div
            className="flex-1 rounded-[28px] border border-zinc-800 bg-zinc-950 bg-cover bg-center"
        ...
        <div className="flex min-h-0 flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-3 rounded-2xl border border-zinc-800 bg-zinc-950/75 p-3">
            <Button type="button" variant="outline" onClick={() => void handleKeep()} disabled={retainedAssets.length === 0}>
              <CheckCircle2 className="h-4 w-4" />
              保留
            </Button>
            <Button type="button" variant="outline" onClick={handleSendToImageLab} disabled={selectedAssets.length === 0}>
            ...
            <Button type="button" variant="outline" onClick={() => void handleDiscard()} disabled={selectedAssets.length === 0}>
