...
### Fix (修复)
- **Gateway Image Health Check**: 本地网关图片能力的健康检查不再触发真实 Flow 生图请求，只做状态判定，避免为了刷新状态反而改动页面。
- **Flow Create Delay Handling**: Flow 的 Create 点击现在会等待延迟起跑，并避免重复点击；如果请求确实没启动，会给出更明确的失败提示。
